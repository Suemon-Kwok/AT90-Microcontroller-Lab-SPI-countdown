	...
	while (1) {
		...
		if (current_second != last_second) {
			...
			if (countdown_value > 0) {
				countdown_value--; update_display();
				if (countdown_value == 0) {
					current_state = STATE_ALARM; alarm_toggle = false;
				}
			}
		}
		break;
...
case STATE_ALARM:
{
	...
	if (alarm_second != last_second) {
		last_second = alarm_second; alarm_toggle = !alarm_toggle;
		if (alarm_toggle) {
			buzzer_on();
...
uint8_t get_current_second(void) {
	uint8_t seconds_bcd = readRTC(DS1305_READ_CMD | DS1305_SECONDS_REG);
